        ...
        do
            ...
            do
                ...
                then
                    for ep in $eps
                    do
                        ./bandit.sh --instance $name --algorithm $al --randomSeed $r --epsilon $ep --horizon $h >> e1.txt
                        # counter=$((counter+1))
                    done
                else
                    ./bandit.sh --instance $name --algorithm $al --randomSeed $r --epsilon 0.1 --horizon $h >> e1.txt
                    # counter=$((counter+1))
                fi
            done
        done
