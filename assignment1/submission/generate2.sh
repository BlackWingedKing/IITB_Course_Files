...
counter=0
for name in '../instances/i-3.txt'
do
    for al in 'thompson-sampling'
    do
        for h in $hz
        do
            for r in $rs
            do
                ...
                then
                    for ep in $eps
                    do
                        ./bandit.sh --instance $name --algorithm $al --randomSeed $r --epsilon $ep --horizon $h >> 2.txt
                        # counter=$((counter+1))
                    done
                else
                    ./bandit.sh --instance $name --algorithm $al --randomSeed $r --epsilon 0.1 --horizon $h >> 2.txt
                    # counter=$((counter+1))
                fi
            done
        done
