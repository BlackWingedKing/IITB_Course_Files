...
for name in "../instances/i-1.txt" "../instances/i-2.txt" "../instances/i-3.txt"
do
    for al in 'kl-ucb'
    do
        for h in 50 20 800 3200 12800 51200 204800
        do
            for r in {30..34}
            do
                ...
                then
                    for ep in $eps
                    do
                        ./bandit.sh --instance $name --algorithm $al --randomSeed $r --epsilon $ep --horizon $h >> b1.txt
                        # counter=$((counter+1))
                    done
                else
                    ./bandit.sh --instance $name --algorithm $al --randomSeed $r --epsilon 0.1 --horizon $h >> b1.txt
                    # counter=$((counter+1))
                fi
            done
        done
